...
{0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF},/*"~",Solid cursor*/
{0x00,0x10,0x00,0x10,0x00,0x10,0x00,0x10,0x00,0x10,0x00,0x10},/*"~",Underline cursor*/
}; 
...
#define Xcommand(Reg)  I2C_Write_Byte(Reg, IIC_CMD)
#define Xdata(Data)    I2C_Write_Byte(Data, IIC_RAM)
...
void command(uint8_t value)
{
  uint8_t buf[2] = {IIC_CMD, value};
  i2c_send_bytes(0x78, 2, buf);
}

void data(uint8_t value)
{
  uint8_t buf[2] = {IIC_RAM, value};
  i2c_send_bytes(0x78, 2, buf);
}
#endif
...
void SSD1309_begin()
{
  ...
  // I2C already set up
	...
	*/	
    ...
    //i2c_start();
    //i2c_send_byte(0x3c<<1);  //i2c address
...
void SSD1309_char(unsigned char x, unsigned char y, int ascii, char size, char mode)
{
...
void SSD1309_string(uint8_t x, uint8_t y, const char *pString, uint8_t Size, uint8_t Mode)
{
    while (*pString != '\0') {       
        ...
        SSD1309_char(x, y, *pString, Size, Mode);
        x += Size / 2;
        pString ++;
    }
}
