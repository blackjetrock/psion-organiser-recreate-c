...
short picocalc_current_x = 0, picocalc_current_y = 0; // the current default position for the next char to be written
...
/******************************************************************************************
 ...
 The char is printed at the current location defined by picocalc_current_x and picocalc_current_y
*****************************************************************************************/

void picocalc_lcd_print_char( int fc, int bc, char c, int orientation)
{
  ...
  if (c >= fp[2] && c < fp[2] + fp[3])
    ...
    draw_bitmap_spi(picocalc_current_x + modx, picocalc_current_y + mody, width, height, scale, fc, bc, np);
  }
  else
    {
      draw_rect_spi(picocalc_current_x + modx, picocalc_current_y + mody, picocalc_current_x + modx + (width * scale),
                    picocalc_current_y + mody + (height * scale), bc);
    }
  
  if (orientation == ORIENT_NORMAL)
    {
      picocalc_current_x += width * scale;
    }
}
...
void picocalc_display_put_c(char c)
{
  ...
  case '\n':
    picocalc_current_x = 0;
    picocalc_current_y += gui_font_height;
    if (picocalc_current_y + gui_font_height >= vres) {
      scroll_lcd_spi(picocalc_current_y + gui_font_height - vres);
      picocalc_current_y -= (picocalc_current_y + gui_font_height - vres);
    }
    return;
  case '\t':
    do {
...
void picocalc_lcd_clear()
{
  draw_rect_spi(0, 0, hres - 1, vres - 1, PICOCALC_BLACK);
}

void picocalc_lcd_putc(uint8_t c) {
  picocalc_display_put_c(c);
}
...
void picocalc_lcd_init() {
  lcd_spi_init();
  pico_lcd_init();

  set_font();
  gui_fcolour = PICOCALC_GREEN;
  gui_bcolour = PICOCALC_BLACK;
