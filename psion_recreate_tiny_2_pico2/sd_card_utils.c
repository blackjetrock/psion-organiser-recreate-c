...
const char *chk_dflt_log_drv(const size_t argc, const char *argv[]) {
 ...
 void run_setrtc(const size_t argc, const char *argv[]) {
 ...
 char const *fs_type_string(int fs_type) {
    switch (fs_type) {
        case FS_FAT12:
            return "FAT12";
        case FS_FAT16:
...
void run_info(const size_t argc, const char *argv[]) {
 ...
 void run_format(const size_t argc, const char *argv[]) {
    const char *arg = chk_dflt_log_drv(argc, argv);
    if (!arg)
        return;
    sd_card_t *sd_card_p = sd_get_by_drive_prefix(arg);
...
{
    sd_card_t *TULIP_uSD_p = sd_get_by_num(0);
    
    if (!TULIP_uSD_p)
    {
        // not good, cannot get pointer
        printf("Unknown logical drive id");
        return;
    }
    ...
    if (!TULIP_uSD_p->state.mounted) {
        printf("  uSD card is not mounted");
        printf("  mounting uSD card");
        fr = f_mount(fs_p, "", 1);
        if (FR_OK != fr) {
            printf("  f_mount error: %s (%d)", FRESULT_str(fr), fr);
            return;
        }
        TULIP_uSD_p->state.mounted = 1;
        printf("  uSD card is mounted");
    }
    else
    {
        printf("  uSD card is already mounted");
    }
    return;
}

 void run_mount2(const size_t argc, const char *argv[]) {
    const char *arg = chk_dflt_log_drv(argc, argv);
    if (!arg)
        return;
    //sd_card_t *sd_card_p = sd_get_by_drive_prefix(arg);
 ...
 void run_chdrive(const size_t argc, const char *argv[]) {
    const char *arg = chk_dflt_log_drv(argc, argv);
    if (!arg)
        return;
 ...
 void run_pwd(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;
 ...
 void run_cp(const size_t argc, const char *argv[]) {
 ...
 void run_mv(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 2)) return;
...
#if 0
void run_lliot(const size_t argc, const char *argv[]) {
    const char *arg = chk_dflt_log_drv(argc, argv);
    if (!arg)
        return;
 ...
 void run_big_file_test(const size_t argc, const char *argv[]) {
 ...
 void run_del_node(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 1)) return;
    del_node(argv[0]);
}
 void run_rm(const size_t argc, const char *argv[]) {
    if (argc < 1) {
        missing_argument_msg();
        return;
    }
...
void run_simple(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;

    simple();
}
 void run_bench(const size_t argc, const char *argv[]) {
    const char *arg = chk_dflt_log_drv(argc, argv);
    if (!arg)
        return;
 ...
 void run_cdef(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;

    f_mkdir("/cdef");  // fake mountpoint
    vCreateAndVerifyExampleFiles("/cdef");
}
 void run_swcwdt(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;

    vStdioWithCWDTest("/cdef");
}
 void run_loop_swcwdt(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;
 ...
 void run_start_logger(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;
    adc_init();
 ...
 void run_stop_logger(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;

    logger_enabled = false;
}
 void run_mem_stats(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;
...
/* Derived from pico-examples/clocks/hello_48MHz/hello_48MHz.c */
 void run_measure_freqs(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;
 ...
 void run_set_sys_clock_48mhz(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 0)) return;

    set_sys_clock_48mhz();
    setup_default_uart();
}
 void run_set_sys_clock_khz(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 1)) return;
 ...
 void set(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 1)) return;
 ...
 void clr(const size_t argc, const char *argv[]) {
    if (!expect_argc(argc, argv, 1)) return;
 ...
 void run_test(const size_t argc, const char *argv[]) {    
    if (!expect_argc(argc, argv, 0)) return;
 ...
 void run_help(const size_t argc, const char *argv[]);
...
#if 0
// Break command
 void chars_available_callback(void *ptr) {
    (void)ptr;   
 ...
 void process_cmd(size_t cmd_sz, char *cmd) {
    assert(cmd);
    assert(cmd[0]);
